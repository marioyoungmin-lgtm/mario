...
function resolveApiBaseUrl(): string {
  ...
  if (typeof window !== 'undefined' && window.location.hostname !== 'localhost') {
    // On hosted frontends, require explicit env config instead of hard-coding localhost.
    return '';
  }
...
async function ensureDefaultProfileExists(): Promise<void> {
  await fetch(`${resolveApiBaseUrl()}/profiles/`, {
    method: 'POST',
...
export async function fetchDailyPlan(childId: number): Promise<DailyTask[]> {
  const response = await fetch(`${resolveApiBaseUrl()}/daily-plan/${childId}`, {
    cache: 'no-store',
  });
  ...
  if (response.status === 404) {
    ...
    const retry = await fetch(`${resolveApiBaseUrl()}/daily-plan/${childId}`, {
      cache: 'no-store',
    });
  ...
  throw new Error('Backend unavailable. Set NEXT_PUBLIC_API_BASE_URL to your public API URL.');
}
...
export async function completeTask(taskId: number, completed: boolean): Promise<DailyTask> {
  const response = await fetch(`${resolveApiBaseUrl()}/tasks/complete_task`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function saveDailyCheckin(payload: SaveCheckinPayload): Promise<void> {
  const response = await fetch(`${resolveApiBaseUrl()}/daily-plan/${payload.child_id}/checkin`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
