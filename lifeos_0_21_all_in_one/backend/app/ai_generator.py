...
APTITUDES = ["logic", "maths", "biology", "physics", "chemistry", "linguistic"]
    ...
    def generate(
        ...
        *,
        ...
        prompt = (
            ...
            "pillar, title, description, duration_minutes, difficulty_level, aptitude. "
            "difficulty_level must be one of easy|medium|hard. "
            f"aptitude must be one of {APTITUDES}."
        )
    ...
    def generate(
        ...
        *,
        ...
        for pillar in strategy.target_pillars:
            title, description, duration, difficulty, aptitude = seed[pillar]
            tasks.append(
                {
                    "pillar": pillar,
                    "title": title,
...
def _build_aptitude_program_tasks(age: int) -> list[dict]:
    """Build a dedicated set of aptitude programs across core academic domains."""
    base_duration = 14 if age <= 12 else 18
    level = "easy" if age <= 7 else "medium" if age <= 15 else "hard"

    return [
        {
            "pillar": "Cognitive",
            "title": "Logic Program",
            "description": "Work through pattern reasoning and deduction drills.",
        ...
        },
        {
            "pillar": "Cognitive",
            "title": "Maths Program",
            "description": "Practice arithmetic and word-problem solving for daily fluency.",
            "duration_minutes": base_duration + 2,
            "difficulty_level": level,
            "aptitude": "maths",
        },
        {
            ...
            "description": "Explore one living-system topic and explain it with examples.",
            "duration_minutes": base_duration + 2,
            "difficulty_level": level,
            "aptitude": "biology",
        },
        {
            ...
            "description": "Run a simple force, motion, or energy experiment and record results.",
            "duration_minutes": base_duration + 3,
            "difficulty_level": level,
            "aptitude": "physics",
        },
        {
            ...
            "description": "Observe one safe reaction concept and describe material changes.",
            "duration_minutes": base_duration + 3,
            "difficulty_level": level,
            "aptitude": "chemistry",
        },
        {
            "pillar": "Language",
            "title": "Linguistic Program",
            "description": "Read a passage and summarize key ideas using new vocabulary.",
            "duration_minutes": base_duration + 1,
            "difficulty_level": level,
            "aptitude": "linguistic",
        },
    ]
...
def _post_process_tasks(tasks: list[dict], strategy: GenerationStrategy) -> list[dict]:
    ...
    for task in tasks:
        ...
        aptitude = str(task.get("aptitude", "logic")).lower()
        ...
        task["aptitude"] = aptitude if aptitude in APTITUDES else "logic"
...
async def generateDailyTasks(
    ...
    adjusted.extend(_build_aptitude_program_tasks(age))
    return adjusted
